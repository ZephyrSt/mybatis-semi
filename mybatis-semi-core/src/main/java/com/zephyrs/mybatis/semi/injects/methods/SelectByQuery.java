...
import com.zephyrs.mybatis.semi.annotations.query.*;
...
public class SelectByQuery extends AbstractInjectMethod {
    ...
    private String equal(Field field, TableInfo tableInfo) {
        ...
        if (equal != null) {
            bindColumn = equal.value();
        }
        ColumnInfo column = MetadataHelper.getColumnByFieldName(tableInfo, bindColumn);
        if (column == null) {
            return EMPTY_STR;
        }
    ...
    private String between(Field field, TableInfo tableInfo) {
        ...
        String bindColumn = field.getName();
        if (between.value() != null && !between.value().isEmpty()) {
            bindColumn = between.value();
        }
        ColumnInfo column = MetadataHelper.getColumnByFieldName(tableInfo, bindColumn);
        if (column == null) {
            return EMPTY_STR;
    ...
    private String in(Field field, TableInfo tableInfo) {
        ...
        ColumnInfo column = MetadataHelper.getColumnByFieldName(tableInfo, fieldName);
        if (column == null) {
            return EMPTY_STR;
        }
        String script = "<if test=\"query.%s != null and query.%s.size()>0\">" +
                "AND %s IN " +
                "<foreach collection=\"query." + fieldName + "\" item=\"item\" index=\"index\" open=\"(\" close=\")\" separator=\",\">" +
                "#{item}</foreach>" +
                "</if>";
        return String.format(script, fieldName, fieldName, column.getColumnName());
    }
    ...
    private String parseLike(Field field, TableInfo tableInfo, String annotaionValue, String type) {
        String bildField = field.getName();
        if (annotaionValue != null && !annotaionValue.trim().isEmpty()) {
            bildField = annotaionValue;
        }
        ColumnInfo column = MetadataHelper.getColumnByFieldName(tableInfo, bildField);
        if (column == null) {
            return EMPTY_STR;
        }
        String fieldName = field.getName();
